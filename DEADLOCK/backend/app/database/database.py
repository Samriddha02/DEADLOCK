...
def delete_project(owner: str, repo: str) -> bool:
    """
    Delete a project from SQLite by owner/repo.
    ...
    with _connect() as conn:
        cursor = conn.execute(
            "DELETE FROM projects WHERE owner = ? AND repo = ?",
            (owner, repo),
        )
        conn.commit()
...
def list_projects() -> list[dict]:
    """
    Return all stored projects from SQLite.
    """
    init_db()
